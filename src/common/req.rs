...
async fn media_store(media: &mut Media, store: &ShoelaceData) -> Result<(), proxy::Error> {
    media.content = proxy::store(&media.content, store).await?;

    media.thumbnail = proxy::store(&media.thumbnail, store).await?;
...
pub(crate) async fn user<'a>(user: &'a str, store: &ShoelaceData) -> Result<User, Error> {
    ...
    let pfp = proxy::store(resp.pfp.as_str(), store).await?;
...
// pub(crate) async fn post(post: String, store: ShoelaceData) -> Result<Post, Error> {
//     // Fetch post
//     let thread = Threads::new()?;
